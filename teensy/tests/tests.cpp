...
void MeshInternal_ProcessSensorStatusShort(uint8_t * payload, size_t len)
{
  ...
  uint16_t property_id = (payload[SS_SHORT_PROPERTY_ID_LOW_INDEX] & SS_SHORT_PROPERTY_ID_LOW_MASK) >> SS_SHORT_PROPERTY_ID_LOW_OFFSET;
  property_id |= payload[SS_SHORT_PROPERTY_ID_HIGH_INDEX] << 8;

  MeshInternal_ProcessSensorProperty(property_id, payload + SS_SHORT_PROPERTY_OFFSET, message_len - SS_SHORT_PROPERTY_OFFSET);
}
...
void MeshInternal_ProcessSensorStatusLong(uint8_t * payload, size_t len)
{
  ...
  MeshInternal_ProcessSensorProperty(property_id, payload + SS_LONG_PROPERTY_OFFSET, message_len - SS_LONG_PROPERTY_OFFSET);
}
...
void MeshInternal_ProcessPresentAmbientLightLevel(uint8_t * payload, size_t len)
{
  ...
  {
    printf("Received sensor status message, present ambient light level with value of: %d\n", value);
  }
}
